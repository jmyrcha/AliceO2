...
void MultiView::registerGeometry(TEveGeoShape *geom, bool threeD, bool rPhi, bool zRho)
{
  ...
  //cout << "Pushing geometry for: " << geom->GetName() << ", new size: " << mGeomVector.size() << endl;
...
{
  //std::cout << "Destroying geometries, size: " << mGeomVector.size() << std::endl;
  //for(int i = 0; i < mGeomVector.size(); ++i) {
  //    std::cout << "Destroying geometry for: " << mGeomVector[i]->GetName() << std::endl;
