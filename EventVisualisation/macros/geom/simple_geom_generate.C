...
void AddNodes(TGeoNode* node, TEveGeoNode* parent, Int_t depth, Int_t depthmax, TObjArray* list)
{
   ...
   // cout << "Searching for element: " << nname->GetString().Data() << endl;
    ...
    for (int i = 0; i < nlist->GetEntries(); i++) {
        ...
        //cout << "Current node: " << node2->GetName() << endl;
...
void simple_geom_generate(char *detectorName = "", int runNumber = 0, std::string inputGeom = "O2geometry.root")
{
    ...
    // TODO: enable OCDB once GeometryManager will be able to load from it
    // load config file
    // TEnv settings;
    // o2::EventVisualisation::ConfigurationManager::getInstance().getConfig(settings);
    
    // set OCDB path from config and set run number for which we want to generate geometry
    // o2::ccdb::Manager::Instance()->setDefaultStorage(settings.GetValue("OCDB.default.path", "local://$ALICE_ROOT/../src/OCDB"));
    // o2::ccdb::Manager::Instance()->setRun(runNumber);
    
    // load geometry from OCDB
    // o2::EventVisualisation::GeometryManager::LoadGeometry();
    ...
    for(int i = 0; i < detectorsList.size(); i++)
    {
        ...
        while (true)
        {
            ...
            if (!path.Contains("cave")) continue;
        ...
        else
        {
            ...
            //eve_tnode->SaveExtract(Form(/*"../../resources/geometry/*/"simple_geom_%s.root", currentDetector), currentDetector, kTRUE);
        }
    }
}
